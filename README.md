...
### 测试的一般流程 
1. 准备工作  先要确定在什么阶段开展性能测试工作，该使用哪些工具，具体的分工是怎样的。
2. 性能测试计划 要分析性能测试背景，用户场景，性能目标，以及制定性能测试实施计划。
3. 性能测试设计 具体包括了测试环境设计，测试场景设计，测试用例设计 ，编写测试脚本 
4. 性能测试执行 先部署好测试环境 一般由运维或开发人员进行环境的部署，并进行资源协调。然后执行测试脚本按照业务场景和测试用例，按顺序执行我们已经设计好的测试脚本。 接着进行性能监控和记录。
5. 性能测试分析 分析不同的测试环境下，硬件设备的性能指标与预期的性能指标进行对比，确定是否达到了我们需要的结果。
6. 性能测试调优 确定问题：根据性能分析结果确定存在的性能问题。 然后分析问题根据确定的问题进行具体详细的分析出现问题的原因进行调整，使分析调优结果达到预期目标。
7. 性能汇总与报告 对性能测试的过程和结果进行汇总 编写性能测试报告。
